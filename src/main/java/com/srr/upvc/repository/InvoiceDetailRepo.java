...
public interface InvoiceDetailRepo extends JpaRepository<InvoiceInfo, Long> {
	...
	@Query(value="SELECT  SUM(INV.TOTAL_AMOUNT) AS MONTH_TOTAL_AMOUNT,concat(MONTHNAME(INV.INVOICE_DATE), '-' ,EXTRACT(YEAR FROM INV.INVOICE_DATE)) AS MONTH_NAME FROM INVOICE_INFO INV INNER JOIN CUSTOMER_ORDER_INFO CO ON CO.ORDER_ID=INV.ORDER_ID WHERE INV.INVOICE_DATE BETWEEN STR_TO_DATE(:invStartDate, '%d/%m/%Y') and  STR_TO_DATE(:invEndDate, '%d/%m/%Y') AND CO.STATUS NOT IN ('ORDER_NOT_REQUIRED') GROUP BY MONTH_NAME", nativeQuery=true)
